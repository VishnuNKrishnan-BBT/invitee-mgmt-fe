...
import Styles from './UnsureGuestList.module.css'
...
function UnsureGuestList({ onClose = null }) {


    const [guestList, setGuestList] = useState([])

    useEffect(() => {
        requestGetGuests({ attendance: 'unsure' }).then(res => {
            setGuestList(res)
        })
    }, [])
    ...
    return (
        <div className={`${Styles.wrapper}`}>
            <div className={`${Styles.popupHeadingContainer}`}>
                <h1 className={`${Styles.popupHeading}`}>Attendance Unsure</h1>
...
export default UnsureGuestList
