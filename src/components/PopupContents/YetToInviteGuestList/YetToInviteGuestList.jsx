...
import Styles from './YetToInviteGuestList.module.css'
...
function YetToInviteGuestList({ onClose = null }) {


    const [guestList, setGuestList] = useState([])

    useEffect(() => {
        requestGetGuests({ invited: false }).then(res => {
            setGuestList(res)
        })
    }, [])
    ...
    return (
        <div className={`${Styles.wrapper}`}>
            <div className={`${Styles.popupHeadingContainer}`}>
                <h1 className={`${Styles.popupHeading}`}>Yet to Invite</h1>
...
export default YetToInviteGuestList
