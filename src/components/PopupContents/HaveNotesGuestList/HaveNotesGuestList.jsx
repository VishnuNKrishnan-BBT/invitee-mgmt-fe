...
import Styles from './HaveNotesGuestList.module.css'
...
function HaveNotesGuestList({ onClose = null }) {


    const [guestList, setGuestList] = useState([])

    useEffect(() => {
        requestGetGuests({ notes: { $ne: "" } }).then(res => {
            setGuestList(res)
        })
    }, [])
    ...
    return (
        <div className={`${Styles.wrapper}`}>
            <div className={`${Styles.popupHeadingContainer}`}>
                <h1 className={`${Styles.popupHeading}`}>Have Notes</h1>
...
export default HaveNotesGuestList
