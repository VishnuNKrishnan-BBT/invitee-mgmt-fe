...
import Styles from './WillAttendGuestList.module.css'
...
function WillAttendGuestList({ onClose = null }) {


    const [guestList, setGuestList] = useState([])

    useEffect(() => {
        requestGetGuests({ attendance: 'will' }).then(res => {
            setGuestList(res)
        })
    }, [])
    ...
    return (
        <div className={`${Styles.wrapper}`}>
            <div className={`${Styles.popupHeadingContainer}`}>
                <h1 className={`${Styles.popupHeading}`}>Will Attend</h1>
...
export default WillAttendGuestList
